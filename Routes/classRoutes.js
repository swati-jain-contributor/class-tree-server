...
var routes = function () {
  ...
  classRouter.route('/getClasses').post(function (req, res) {
    try {
      if (req.body.type == "T")
        sql = "Select * , (select COUNT(*) from shareskill.Student where ClassId = C.id )  As Attendee from shareskill.Class As C join Tutor t on C.tutorId =t.id where TutorEmail='" + req.body.email + "' order by C.active DESC,  C.DATE ASC";
